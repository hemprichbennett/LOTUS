...
#works
random_nets <- lapply(netlists[[1]], function(x) replicate(1000, permatfull(x, fixedmar=sums_to_preserve,mtype="count",times=1)$perm))

#works
lapply(netlists[[1]], function(x) replicate(1000, networklevel(permatfull(x, fixedmar=sums_to_preserve,mtype="count",times=1)$perm[[1]], index = ind)))
